...
주간 사용량(n/N회), 여러 수업 일괄 신청, 공지사항·운영시간 확인, 다크/라이트 토글, PWA 설치
...
관리자 계정 관리, 통계 대시보드, 코치별 보기, 회원 대리예약·보강, 슬롯 정원 수정, 수업 일괄 삭제
...
| 고정 수업 | 슬롯 생성 시 자동 예약. 위 규칙을 그대로 통과해야 함.<br/>생성 시점에 이용권이 없으면 건너뛰지만, Cron이 주 1회 재시도하므로 자동 복구된다 |
| 보강 | 관리자 대리예약 `force=true` — 1일1회·주간횟수만 건너뛴다. 이용권·정원은 유지 |
...
- **통계 기간에 미래를 넣을 것** — 예약은 대부분 앞으로의 날짜다. 과거 30일만 집계하면
  실제 예약 현황과 안 맞는다.
- **함수 GRANT는 생성 시점에 걸린다** — `grant on all functions`는 그 시점 함수만 커버한다.
  이후 만든 함수를 PUBLIC에서 revoke하면 service_role도 권한을 잃는다(0014에서 default privileges로 해결).
